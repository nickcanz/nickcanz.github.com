var App = (function () {
  ...
  self.generateTableRow = function (setting) {
    ...
    var cells = `
      ...
      <td>${(name.length < 60) ? name : "<abbr title=\"" + name + "\">" + name.substring(0,100) + "</abbr>" }</td>
      <td>${setting["JavaType"]}</td>
      <td>${(setting["DefaultArg"].length < 60) ? setting["DefaultArg"] : "<abbr title=\"" + setting["DefaultArg"] + "\">" + setting["DefaultArg"].substring(0, 100) + "</abbr>"}</td>
